...
interface MediaPreviewProps {
  ...
  rounded?: boolean; // control rounded corners on the preview container
  chrome?: boolean; // show the top window chrome bar (dots)
}
...
export const MediaPreview: React.FC<MediaPreviewProps> = ({
  ...
  return (
    <div
      className={`relative w-full h-full overflow-hidden ${rounded ? 'rounded-lg' : 'rounded-none'} border border-border bg-foreground/[0.04] dark:bg-foreground/[0.06] shadow-[0_1px_0_0_rgba(0,0,0,0.08),inset_0_0_0_1px_rgba(255,255,255,0.03)]`}
    >
      {/* Window chrome */}
      {chrome && (
      ...
      {/* Image area */}
      <div className={`absolute inset-x-0 bottom-0 ${chrome ? 'top-5 sm:top-6' : 'top-0'}`}>
        <Image
          src={src}
          alt={alt}
          fill
